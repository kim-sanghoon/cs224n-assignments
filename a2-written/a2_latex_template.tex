...
\newcommand{\hmwkAuthorName}{Sanghoon Kim} % Your name
...
\begin{enumerate}[label=(\roman*)]
    \item
        \begin{align*}
            \frac{\partial \bm J}{\partial \bm v_c}
            &= - \frac{\sigma'(\bm u_o^\top \bm v_c)}{\sigma(\bm u_o^\top \bm v_c)} \cdot \bm{u}_o
               - \sum_{s=1}^K \frac{\sigma'(-\bm u_{w_s}^\top \bm v_c)}{\sigma(-\bm u_{w_s}^\top \bm v_c)} \cdot \bm{u_{w_s}} \\
            &= (\sigma(\bm u_o^\top \bm v_c) - 1) \cdot \bm{u}_o + \sum_{s=1}^K \sigma(\bm u_{w_s}^\top \bm v_c) \cdot \bm{u_{w_s}}
        \end{align*}
        \begin{align*}
            \frac{\partial \bm J}{\partial \bm u_o}
            &= - \frac{\sigma'(\bm u_o^\top \bm v_c)}{\sigma(\bm u_o^\top \bm v_c)} \cdot \bm{v}_c \\
            &= (\sigma(\bm u_o^\top \bm v_c) - 1) \cdot \bm{v}_c
        \end{align*}
        \begin{align*}
            \frac{\partial \bm J}{\partial \bm u_{w_s}}
            &= \frac{\sigma'(-\bm u_{w_s}^\top \bm v_c)}{\sigma(-\bm u_{w_s}^\top \bm v_c)} \cdot \bm{v_{c}} \\
            &= (1 - \sigma(-\bm u_{w_s}^\top \bm v_c)) \cdot \bm{v_c}
        \end{align*}
    \item Because $1 - \sigma(\bm u_o^\top \bm v_c)$ and $1 - \sigma(-\bm u_{w_s}^\top \bm v_c)$ (where $s \in [1, K]$) are reused over the three derivatives, we can use $\bm{1} - \sigma(\bm{U}\bm{v}_c)$ to minimize computation.
    \item While naive-softmax loss requires a softmax operation over entire \text{Vocab}, negative sampling loss only requires computation over smaller negative samples.
\end{enumerate}
...
First, decompose the sum into two:
\begin{equation*}
    \bm{J}
    = -\log(\sigma(\bm u_o^\top \bm v_c))
    - \sum_{\substack{k \in [1, K] \\ w_k = w_s}} \log(\sigma(-\bm u_{w_k}^\top \bm v_c))
    - \sum_{\substack{k \in [1, K] \\ w_k \ne w_s}} \log(\sigma(-\bm u_{w_k}^\top \bm v_c))
\end{equation*}

Calculating the gradient $\frac{\partial \bm J}{\partial \bm u_{w_s}}$,
\begin{align*}
    \frac{\partial \bm J}{\partial \bm u_{w_s}}
    &= \sum_{\substack{k \in [1, K] \\ w_k = w_s}} \sigma(\bm u_{w_k}^\top \bm v_c) \cdot \bm{v}_c \\
    &= \sigma(\bm u_{w_s}^\top \bm v_c) \cdot \bm{v}_c \cdot \sum_{\substack{k \in [1, K] \\ w_k = w_s}} 1
\end{align*}
...
\begin{figure}[H]
    ...
    \caption{Generated word vectors visualization from my implementation}
    \label{fig:word-vector}
\end{figure}

\begin{enumerate}
    \item The relationship of \texttt{male} and \texttt{female} is similar to that of \texttt{king} and \texttt{queen}.
    \item However, the relationship of \texttt{man} and \texttt{woman} is different to those of examples above.
    \item The distance of \texttt{enjoyable} and \texttt{boring} is similar to that of \texttt{wonderful} and \texttt{annoying}, while the directions are opposite.
\end{enumerate}
