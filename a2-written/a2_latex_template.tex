...
\begin{enumerate}[label=(\roman*)]
    \item
        ...
        \begin{align*}
            ...
            &= - \sum_{s=1}^K \frac{\sigma'(-\bm u_{w_s}^\top \bm v_c)}{\sigma(-\bm u_{w_s}^\top \bm v_c)} \cdot \bm{v_{c}} \\
            &= \sum_{s=1}^K \sigma(\bm u_{w_s}^\top \bm v_c) \cdot \bm{v_c}
        \end{align*}
    \item Because $1 - \sigma(\bm u_o^\top \bm v_c)$ and $\sum_{s=1}^K 1 - \sigma(-\bm u_{w_s}^\top \bm v_c)$ are reused over the three derivatives, we can use $\bm{1} - \bm{U}\bm{v}_c$ to minimize computation.
...
Calculating the gradient $\frac{\partial \bm J}{\partial \bm w_s}$,
\begin{align*}
    \frac{\partial \bm J}{\partial \bm w_s}
